...
import { seedStore, transactionStore, type Transaction } from "@/lib/store"
  ...
  return (
    <React.Fragment>
      ...
      >
        ...
        <td className="px-4 py-3">
          <span className={cn("text-xs font-semibold px-2 py-1 rounded-md capitalize", GATEWAY_BADGE[txn.gateway] ?? "bg-muted text-muted-foreground")}>
            {txn.gateway}
          </span>
        </td>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              <div>
                ...
                <pre className="text-xs font-mono bg-card border border-border rounded-lg p-3 overflow-auto max-h-32 text-muted-foreground">
                  {JSON.stringify(txn.rawResponse, null, 2)}
                </pre>
              </div>
...
function TransactionsContent() {
  ...
  const filtered = txns.filter(t => {
    const matchSearch  = !search || t.referenceId.toLowerCase().includes(search.toLowerCase()) || t.id.toLowerCase().includes(search.toLowerCase())
  ...
  const totalSuccess = txns.filter(t => t.status === "success").reduce((s, t) => s + (t.currency === "USD" ? t.amount : 0), 0)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <p className="text-sm text-muted-foreground mb-8">Complete audit trail of all payment gateway transactions. Click a row to see raw response.</p>

      {/* KPI row */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
        {[
          { label: "Total Transactions", value: txns.length,                                        icon: Activity,   color: "text-brand",     bg: "bg-brand/10" },
          { label: "Successful (USD)",   value: `$${totalSuccess.toFixed(2)}`,                      icon: DollarSign, color: "text-green-600 dark:text-green-400",  bg: "bg-green-50 dark:bg-green-900/20" },
          { label: "Pending",            value: txns.filter(t => t.status === "pending").length,    icon: Clock,      color: "text-amber-600 dark:text-amber-400", bg: "bg-amber-50 dark:bg-amber-900/20" },
          { label: "Refunded",           value: txns.filter(t => t.status === "refunded").length,   icon: RefreshCw,  color: "text-blue-600 dark:text-blue-400",   bg: "bg-blue-50 dark:bg-blue-900/20" },
        ].map(kpi => (
