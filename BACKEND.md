...
| 2026-04-12 | Fresh book seed + S3 migration | `scripts/seed-fresh-books.md`, multiple pages | **ACTION REQUIRED:** Clear all existing books, re-seed 30 fresh books with S3-hosted covers + EPUBs + audio. See `scripts/seed-fresh-books.md` for full SQL, S3 upload commands, and verification steps. All book covers must have valid `cover_url` pointing to S3/CDN. |
...
| 2026-04-12 | Author my-books: more response shapes + resilient fetches | `lib/author-my-books.ts`, `lib/book-mapper.ts` (export `unwrapBookRow`), author dashboard / books / sales | **No API change** — accept bare JSON arrays, `books`/`items`/`results` keys, nested `data.items`, Laravel `{ data: { attributes } }` rows; map `status` → approval; treat `live`/`published`/`active` as published. `listMine` failures no longer reject the whole author dashboard `Promise.all`. |
